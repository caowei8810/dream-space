...
import { logRequest, recordRequest, requestMetricSnapshot, resetRequestMetrics, safeRequestPath } from "../src/observability";

afterEach(() => { resetRequestMetrics(); vi.restoreAllMocks(); });
...
describe("HTTP observability", () => {
  it("records request totals by method and status class", () => {
    ...
    expect(requestMetricSnapshot()).toEqual({ total: 3, errors: 1, byMethod: { GET: 1, POST: 2 }, byStatusClass: { "2xx": 1, "5xx": 1, "4xx": 1 } });
  });

  it("logs only safe request metadata", () => {
    const spy = vi.spyOn(Logger.prototype, "log").mockImplementation(() => undefined);
    logRequest({ requestId: "req-1", method: "GET", path: "/health", statusCode: 200, durationMs: 4 });
