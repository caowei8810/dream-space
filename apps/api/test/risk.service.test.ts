...
describe("RiskService", () => {
  it("allows prompts without an active rule hit", async () => {
    ...
    await expect(
      new RiskService(repository as unknown as RiskRepository).inspectPrompt(
        "user-1",
        "晴天海边",
        "request-1",
      ),
  ...
  it("rejects a matched prompt without calling a provider", async () => {
    ...
    await expect(
      service.inspectPrompt("user-1", "包含敏感词的提示", "request-1"),
    ).rejects.toBeInstanceOf(ForbiddenException);
    expect(repository.recordHitAndApply).toHaveBeenCalledWith(
      expect.objectContaining({ requestId: "request-1", inputLength: 8 }),
    );
  });

  it("routes manual review to a conflict response", async () => {
    const { service } = createService("MANUAL_REVIEW");
    await expect(service.inspectPrompt("user-1", "敏感词", "request-2")).rejects.toBeInstanceOf(
      ConflictException,
    );
  });
