...
function createService() {
  ...
  const repository = {
    ...
    createAppeal: vi.fn().mockResolvedValue({ id: "appeal-1", taskId: "task-1", resultId: null, reason: "希望复核", status: "OPEN", decisionNote: null, createdAt: new Date(), decidedAt: null }),
    listUserAppeals: vi.fn().mockResolvedValue([]),
    listAppeals: vi.fn().mockResolvedValue([]),
    reviewResultId: vi.fn(),
    decideAppeal: vi.fn(),
  };
  return { repository, queue, service: new ModerationService(repository as unknown as ModerationRepository, queue as never) };
}

describe("ModerationService", () => {
  it("claims reviews through an atomic repository operation", async () => {
    const { repository, service } = createService();
    await service.claimReview("review-1", actor);
    expect(repository.claimReview).toHaveBeenCalledWith("review-1", "admin-1");
  });
  ...
  it("rejects a second decision when the review is no longer claimed by the actor", async () => {
    ...
    await expect(service.decideReview("review-1", { decision: "approved", note: "复核通过" }, actor)).rejects.toBeInstanceOf(ConflictException);
  });

  it("requeues an input review after manual approval", async () => {
    const { repository, queue, service } = createService();
    vi.mocked(repository.decideReview).mockResolvedValue({ reviewId: "review-1", taskId: "task-1", taskStatus: "queued", shouldEnqueue: true });
  ...
  it("requires an appeal target and normalizes the reason", async () => {
    ...
    expect(repository.createAppeal).toHaveBeenCalledWith("user-1", { taskId: "task-1", reason: "希望复核" });
  });
});
