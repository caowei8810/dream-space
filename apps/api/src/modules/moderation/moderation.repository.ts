...
export class ModerationRepository {
  ...
  async listReviews(input: { page: number; pageSize: number; status?: "OPEN" | "CLAIMED" | "APPROVED" | "REJECTED" }) {
  ...
  async claimReview(id: string, adminId: string) {
    const changed = await this.database.moderationReview.updateMany({
      where: { id, status: "OPEN", assignedToId: null },
      data: { status: "CLAIMED", assignedToId: adminId, claimedAt: new Date() },
    });
    if (changed.count !== 1) return null;
    return this.database.moderationReview.findUnique({ where: { id }, include: reviewInclude });
  }

  async decideReview(id: string, adminId: string, decision: "APPROVED" | "REJECTED", note: string) {
    return this.database.$transaction(async (transaction) => {
      ...
      const now = new Date();
      const taskStatus = review.stage === "INPUT"
        ? (decision === "APPROVED" ? "QUEUED" : "FAILED")
      ...
      const task = await transaction.generationTask.update({
        ...
        data: {
          status: taskStatus,
          ...(review.stage === "INPUT" ? { inputModerationStatus: moderationStatus } : { outputModerationStatus: moderationStatus }),
          ...(taskStatus === "FAILED" || taskStatus === "SUCCEEDED" ? { completedAt: now } : {}),
          errorCode: taskStatus === "FAILED" ? `MANUAL_${review.stage}_MODERATION_${decision}` : null,
      ...
      if (taskStatus === "FAILED") {
        const quota = await transaction.quotaAccount.update({
          ...
          data: { available: { increment: task.totalCost }, reserved: { decrement: task.totalCost } },
        });
        await transaction.quotaLedgerEntry.upsert({
          where: { idempotencyKey: `manual-release:${task.id}` },
          create: { userId: task.userId, taskId: task.id, type: "RELEASE", amount: task.totalCost, balanceAfter: quota.available, idempotencyKey: `manual-release:${task.id}` },
          update: {},
        });
        await this.settlePaidBilling(transaction, task, "release");
      } else if (taskStatus === "SUCCEEDED") {
        const quota = await transaction.quotaAccount.update({ where: { userId: task.userId }, data: { reserved: { decrement: task.totalCost } } });
        await transaction.quotaLedgerEntry.upsert({
          where: { idempotencyKey: `consume:${task.id}` },
          create: { userId: task.userId, taskId: task.id, type: "CONSUME", amount: task.totalCost, balanceAfter: quota.available, idempotencyKey: `consume:${task.id}` },
          update: {},
        });
      ...
      await transaction.moderationReview.update({
        where: { id: review.id },
        data: { status: decision, decision: decision === "APPROVED" ? "approved" : "rejected", decisionNote: note, decidedAt: now },
      });
      await transaction.generationTaskEvent.create({
        data: { taskId: task.id, type: `task.manual_${review.stage.toLowerCase()}_${decision.toLowerCase()}`, status: taskStatus, payload: { reviewId: review.id, decision, adminId } },
      });
      return { reviewId: review.id, taskId: task.id, taskStatus: taskStatus.toLowerCase(), shouldEnqueue: taskStatus === "QUEUED" };
    });
  }
  ...
  private async settlePaidBilling(
    ...
    if (task.cashReservedCents > 0) {
      ...
      const existing = await transaction.cashLedgerEntry.findUnique({ where: { idempotencyKey: key } });
      if (!existing) {
        const changed = await transaction.cashAccount.updateMany({
          where: { userId: task.userId, reserved: { gte: task.cashReservedCents } },
          data: mode === "release"
            ? { reserved: { decrement: task.cashReservedCents }, available: { increment: task.cashReservedCents } }
        ...
        await transaction.cashLedgerEntry.create({
          data: { userId: task.userId, taskId: task.id, type: mode === "release" ? "RELEASE" : "CONSUME", amount: task.cashReservedCents, balanceAfter: account.available, idempotencyKey: key },
        });
      }
    }
    ...
    for (const entry of entries) {
      ...
      const existing = await transaction.entitlementLedgerEntry.findUnique({ where: { idempotencyKey: key } });
      if (existing) continue;
      const changed = await transaction.userEntitlement.updateMany({
        where: { id: entry.entitlementId, reserved: { gte: entry.amount } },
        data: mode === "release" ? { reserved: { decrement: entry.amount }, available: { increment: entry.amount } } : { reserved: { decrement: entry.amount } },
      });
      if (changed.count !== 1) throw new Error("ENTITLEMENT_SETTLEMENT_STATE_INVALID");
      const entitlement = await transaction.userEntitlement.findUniqueOrThrow({ where: { id: entry.entitlementId } });
      await transaction.entitlementLedgerEntry.create({
        data: { userId: task.userId, entitlementId: entry.entitlementId, taskId: task.id, type: mode === "release" ? "RELEASE" : "CONSUME", amount: entry.amount, balanceAfter: entitlement.available, idempotencyKey: key },
      });
    }
  }

  async createAppeal(userId: string, input: { taskId?: string; resultId?: string; reason: string }) {
    return this.database.$transaction(async (transaction) => {
      if (input.taskId) {
        const task = await transaction.generationTask.findFirst({ where: { id: input.taskId, userId }, select: { id: true } });
        if (!task) return null;
      }
      if (input.resultId) {
        const result = await transaction.generationResult.findFirst({ where: { id: input.resultId, task: { userId } }, select: { id: true } });
  ...
  async decideAppeal(id: string, adminId: string, decision: "ACCEPTED" | "REJECTED", note: string) {
    return this.database.$transaction(async (transaction) => {
      const appeal = await transaction.moderationAppeal.findUnique({
        where: { id },
        include: { task: { select: { moderationReviews: { select: { assignedToId: true } } } }, result: { select: { moderationReviews: { select: { assignedToId: true } } } } },
      });
      if (!appeal || appeal.status !== "OPEN") return null;
      const reviews = [...(appeal.task?.moderationReviews ?? []), ...(appeal.result?.moderationReviews ?? [])];
