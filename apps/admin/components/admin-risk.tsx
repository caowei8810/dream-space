  ...
  return (
    <main className="admin-page">
      ...
      <div className="admin-list-summary">
        <span>共 {data.total} 个规则版本</span>
        <span>已发布规则才会拦截生成</span>
      </div>
      <section className="admin-table-region" aria-busy={loading} aria-label="提示词风控规则">
        {loading ? (
          <AdminState kind="loading" title="正在加载风控规则" />
        ) : data.items.length === 0 ? (
          <AdminState kind="empty" title="暂无风控规则" />
        ) : (
              ...
              {data.items.map((rule) => (
                <tr key={rule.id}>
                  <td>
                    ...
                    <small>
                      {rule.code} · v{rule.version} · {rule.category}
                    </small>
                  </td>
                  <td>
                    <span className="admin-status">
                      {rule.matchType === "keyword" ? "关键词" : "正则"}
                    </span>
                    <small className="admin-risk-pattern">{rule.pattern}</small>
                  </td>
                  <td>{actionLabels[rule.action]}</td>
                  <td>{rule.priority}</td>
                  <td>
                    <span
                      className={`admin-status ${rule.status === "published" ? "user-active" : ""}`}
                    >
                  ...
                  <td className="admin-table-action-cell">
                    {canPublish && rule.status === "draft" ? (
                      <button
                        ...
                        type="button"
                        disabled={saving}
                        onClick={() => void updateRule(rule.id, "publish")}
                      >
                        发布
                      </button>
                    ) : null}
                    {canPublish && rule.status === "published" ? (
                      <button
                        className="admin-button secondary"
                        type="button"
                        disabled={saving}
                        onClick={() => void updateRule(rule.id, "archive")}
                      >
                        下线
                      </button>
                    ) : null}
        ...
        <div className="admin-modal-backdrop" role="presentation">
          ...
          >
            <header className="admin-modal-header">
              <div>
                <p className="admin-page-kicker">规则版本</p>
                <h2 id="risk-create-title">新建风控规则</h2>
              </div>
              <button
            ...
            <form className="admin-form-grid" onSubmit={create}>
              ...
              <label>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setForm({ ...form, matchType: e.target.value as RiskRuleMatchType })
                  }
                >
                  {riskRuleMatchTypes.map((value) => (
                    <option key={value} value={value}>
                      {value === "keyword" ? "关键词" : "正则"}
                    </option>
                  ))}
                </select>
              </label>
              <label>
                ...
                <select
                  value={form.action}
                  onChange={(e) => setForm({ ...form, action: e.target.value as RiskAction })}
                >
                  {riskActions.map((value) => (
                    <option key={value} value={value}>
                      {actionLabels[value]}
                    </option>
                  ))}
              ...
              <label className="admin-form-full">
                ...
                <textarea
                  ...
                  placeholder="关键词每行一个；正则仅允许安全表达式"
                />
              </label>
              <label className="admin-form-full">
                <span>操作原因</span>
      ...
      <div className="admin-risk-note">
        <ShieldAlert aria-hidden="true" />
        <span>命中记录仅保留输入哈希、长度、规则版本和处置结果；原始提示词不写入风控命中表。</span>
      </div>
    </main>
  );
}
